...
sed "s|^\(.*<release>One\.\).*\(</release>\)|\1$TS\2|" d.xml > deployment.xml
zip -r -9 ZSST_IBMi_One.${TS}.zpk data scripts read.txt ZSST_icon.png deployment.xml

popd
mv $PackDir/ZSST_IBMi_One.${TS}.zpk .
